...
import { sculptorGuideDocs, sculptorNpmOrgUrl } from '../data'
...
function isBlockStart(line: string) {
  return /^#{1,6}\s+/.test(line) || line.startsWith('```') || line.startsWith('> ') || isListItem(line) || line.includes('|')
}
  ...
  for (const match of text.matchAll(pattern)) {
    ...
    if (full.startsWith('`')) {
      ...
      if (linkMatch) {
        ...
        parts.push(
          <a key={`${start}-link`} href={resolvedHref} target={resolvedHref.startsWith('http') ? '_blank' : undefined} rel={resolvedHref.startsWith('http') ? 'noreferrer' : undefined}>
            {linkMatch[1]}
          </a>,
        )
      } else {
    ...
    <div className="sculptor-markdown">
      {blocks.map((block, blockIndex) => {
        ...
        if (block.type === 'heading') {
          ...
          return (
            <HeadingTag key={key} className={`sculptor-markdown__heading sculptor-markdown__heading--${block.level}`}>
            ...
            <div key={key} className="sculptor-markdown__code-shell">
              ...
              <pre className="sculptor-markdown__code">
                {codeLines.map((codeLine, codeIndex) => (
                  <div key={`${key}-code-${codeIndex}`} className="sculptor-markdown__code-line">
                    <span className="sculptor-markdown__code-no">{codeIndex + 1}</span>
                    <code>{codeLine || ' '}</code>
                  </div>
                ))}
              </pre>
                ...
                <thead>
                  <tr>
                    {block.header.map((cell, cellIndex) => (
                      <th key={`${key}-head-${cellIndex}`}>{renderInline(cell)}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {block.rows.map((row, rowIndex) => (
                    <tr key={`${key}-row-${rowIndex}`}>
                      {row.map((cell, cellIndex) => (
                        <td key={`${key}-row-${rowIndex}-cell-${cellIndex}`}>{renderInline(cell)}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
...
type MarkdownDocCardProps = {
  ...
  summary: string
  path: string
  onStatsChange: (stats: { title: string; words: number; readTime: number }) => void
  isActive: boolean
}

function MarkdownDocCard({ title, summary, path, onStatsChange, isActive }: MarkdownDocCardProps) {
  ...
  return (
    <article className={`sculptor-guide-doc ${isActive ? 'sculptor-guide-doc--active' : ''}`} id={anchorId} data-doc-title={title}>
      <header className="sculptor-guide-doc__header">
        <div>
          <p className="sculptor-guide-doc__eyebrow">Loaded from `public/Sculptor`</p>
      ...
      <div className="sculptor-guide-doc__body">
        {status === 'loading' ? <p className="sculptor-guide-doc__state">Loading markdown...</p> : null}
        {status === 'error' ? <p className="sculptor-guide-doc__state sculptor-guide-doc__state--error">Could not load this markdown file.</p> : null}
...
export function SculptorGuidePage({ contribution, onBackProduct, onOpenContributions }: SculptorGuidePageProps) {
  const { ref, inView } = useInViewReveal({ threshold: 0.1, once: true })
  const [docStats, setDocStats] = useState<Array<{ title: string; words: number; readTime: number }>>([])
  const [activeDocTitle, setActiveDocTitle] = useState(sculptorGuideDocs[0]?.title ?? '')
  const totalReadTime = useMemo(
    () => docStats.reduce((sum, item) => sum + item.readTime, 0),
    [docStats],
  )
  const totalWords = useMemo(() => docStats.reduce((sum, item) => sum + item.words, 0), [docStats])
  const activeDocId = createDocAnchorId(activeDocTitle)

  useEffect(() => {
    const updateActiveDoc = () => {
      const cards = Array.from(document.querySelectorAll<HTMLElement>('.sculptor-guide-doc'))
      if (!cards.length) {
        return
      }
  ...
  return (
    ...
    >
      ...
      >
        <motion.article variants={revealItem} className="sculptor-guide-page__hero">
          <div className="sculptor-guide-page__copy">
            <p className="sculptor-guide-page__status">Beta</p>
            <h2 className="sculptor-guide-page__title">The full framework docs, rendered in-page.</h2>
            <p className="sculptor-guide-page__text">
              This guide is not a separate rewrite. It is a styled wrapper around the actual markdown files from the public docs folder, so the content stays faithful to the source while fitting the portfolio aesthetic.
            </p>
          </div>
        ...
        <div className="sculptor-guide-page__layout">
          <motion.aside variants={revealItem} className="sculptor-guide-page__sidebar">
            <div className="sculptor-guide-page__sidebar-main">
              ...
              <div className="sculptor-guide-page__sidebar-section">
                ...
                <div className="sculptor-guide-page__stats">
                  <div>
                    <span className="sculptor-guide-page__stat-label">Docs</span>
                    <strong>{sculptorGuideDocs.length}</strong>
                  </div>
                  <div>
                    <span className="sculptor-guide-page__stat-label">Words</span>
                    <strong>{totalWords || 'Loading'}</strong>
                  </div>
                  <div>
                    <span className="sculptor-guide-page__stat-label">Read time</span>
                    <strong>{totalReadTime ? `${totalReadTime} min` : 'Loading'}</strong>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="sculptor-guide-page__sidebar-footer">
              <p>This wrapper keeps the docs in the portfolio shell while preserving the markdown source and links.</p>
              <div className="sculptor-guide-page__bottom-links">
                <a href="https://github.com/imprakhartripathi/Sculptor" target="_blank" rel="noreferrer" className="link-btn link-btn--ghost">
                  GitHub
                </a>
            ...
            {sculptorGuideDocs.map((doc) => (
              <motion.div key={doc.path} variants={revealItem}>
                <MarkdownDocCard
                  ...
                  onStatsChange={(stats) =>
                    setDocStats((current) => {
                      const filtered = current.filter((item) => item.title !== stats.title)
